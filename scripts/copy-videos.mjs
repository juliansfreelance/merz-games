 ...
 * 2. resources/videos/ (masters locales, gitignored)
 * 3. public/content/videos/ ya poblado
 *
 * Mapeo mínimo desde masters históricos Radiesse.mp4 / Ultherapy.mp4
 * hacia los nombres del content-manifest. Si hay un pack completo
 * (general1, radiesse, radiesse2, …) se copia 1:1 por basename.
 *
...
/** Alias de masters → destinos del catálogo. */
const MASTER_MAP = {
  'Radiesse.mp4': ['radiesse.mp4', 'radiesse2.mp4', 'general1.mp4'],
  'radiesse.mp4': ['radiesse.mp4'],
  'radiesse2.mp4': ['radiesse2.mp4'],
  'Ultherapy.mp4': ['ultherapy.mp4', 'ultherapy2.mp4'],
  'ultherapy.mp4': ['ultherapy.mp4'],
  'ultherapy2.mp4': ['ultherapy2.mp4'],
...
for (const dir of candidates) {
  ...
  if (dir === destDir && files.every((f) => REQUIRED.includes(basename(f)))) {
    console.log(`[copy-videos] destino ya tiene ${files.length} MP4; nada que hacer`);
    break;
  }
  for (const file of files) {
    // No re-copiar desde el propio destino hacia sí mismo salvo mapeo de masters
...
if (missing.length) {
  const msg = `[copy-videos] faltan: ${missing.join(', ')}. Coloca masters en resources/videos/ o define CONTENT_VIDEOS_DIR.`;
  if (strict) {
    console.error(msg);
    process.exit(1);
  }
